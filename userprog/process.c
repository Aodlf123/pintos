...
duplicate_pte(uint64_t *pte, void *va, void *aux)
{
	...
	writable = is_writable(pte) == 0 ? false : true;
...
__do_fork(void *aux)
{
	...
	//	준용 변경 ( 초기화 해줌 ) 이자식 바꿔야함
	struct intr_frame *parent_if = parent->forkFrame;
	bool succ = true;
	...
	while (i < FD_MAX)
	{
		...
		{
			current->descriptors[i] = file_duplicate(parent->descriptors[i]);
			if (current->descriptors[i] == NULL)
			{
				goto error;
			}
		}
...
load(const char *file_name, struct intr_frame *if_)
{
	...
	//	준용 추가 & 아래 파일 open 하는 로직의 file_name 변수를 exeName 으로 변경함
	char *save;
	char *fn_copy = palloc_get_page(0);
	strlcpy(fn_copy, file_name, PGSIZE);
	char *exeName = strtok_r(fn_copy, " ", &save);
	...
	for (token = strtok_r(file_name, " ", &savePtr); token != NULL; token = strtok_r(NULL, " ", &savePtr))
	{
		...
		argv[index] = if_->rsp;
		strlcpy(if_->rsp, token, maxBuff);
		index++;
	}

	memset((void *)(if_->rsp & ~7ULL), 0, if_->rsp - (if_->rsp & ~7ULL));
	if_->rsp = if_->rsp & ~7ULL;
	...
	{
		if_->rsp -= sizeof(char *);
		*(uintptr_t *)if_->rsp = argv[index];
		index--;
	}

	if_->R.rsi = if_->rsp;
	if_->rsp -= sizeof(void *);
	*(uintptr_t *)if_->rsp = (void *)0;
